...
const schemaObject: { [key in LoginFormFields]: Schema<string | boolean | undefined> } = {
  email: string().email('Invalid email').required(REQUIRED_FIELD_ERROR),
  password: string().min(6, 'Password is too short').matches(PASSWORD_REGEX).required(REQUIRED_FIELD_ERROR),
